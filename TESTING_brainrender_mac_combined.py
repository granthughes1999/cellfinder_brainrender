...
def run_brainrender(cellfinder_output_path, mouseid, brain_regions_to_evalutate, allen_mouse_10um):
    ...
    print(cells_path)
    # Load the .npy file
    points = np.load('/Users/grant/Desktop/mock_cellfinder_output/points/points.npy')

        # Transpose the array
    points = points.T

    # Shuffle the values at each index location
    for i in range(points.shape[0]):
        points[i] = np.random.permutation(points[i])

    # Transpose the array back
    points = points.T

    # Save out shuffled points
    # Save the array to a .npy file
    randomized_cells_path = cellfinder_output_path + 'points/randomized_points.npy'
    np.save(randomized_cells_path, points)
    ...
    # create points actor
    cells = Points(randomized_cells_path)
