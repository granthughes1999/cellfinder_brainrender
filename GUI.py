...
checkbutton = tk.Checkbutton(root, text="Show Labels", variable=checkbutton_var, command=on_checkbutton_click)
...
cellfinder_output_path_entry = tk.Entry(root, textvariable=cellfinder_output_path, width=110)
...
label_cellfinder_output_path = tk.Label(root, text="Enter path to cellfinder output data ")
...
brain_regions_to_evalutate_entry = tk.Entry(root, textvariable=brain_regions_to_evalutate, width=20)
label_brain_regions_to_evalutate = tk.Label(root, text="Enter number of top labled brain regions you want to render ")
...
allen_mouse_10um_entry = tk.Entry(root, textvariable=allen_mouse_10um, width=50)
allen_mouse_10um_entry.insert(0, '/Users/grant/brainglobe/allen_mouse_10um')

label_allen_mouse_10um = tk.Label(root, text="Enter path to you local allen brain atlas ")

estim_shank_radius_um = tk.IntVar()
estim_shank_radius_um_entry = tk.Entry(root, textvariable=estim_shank_radius_um, width=20)
estim_shank_radius_um_entry.insert(tk.END, str(50))
label_estim_shank_radius_um = tk.Label(root, text="Enter estim shank radius in (um) ")


estim_tip_radius_um = tk.IntVar()
estim_tip_radius_um_entry = tk.Entry(root, textvariable=estim_tip_radius_um, width=20)
estim_tip_radius_um_entry.insert(tk.END, str(10))
label_estim_tip_radius_um  = tk.Label(root, text="Enter estim tip radius in (um)")



estim_propigation_radius_um = tk.IntVar()
estim_propigation_radius_um_entry = tk.Entry(root, textvariable=estim_propigation_radius_um, width=20)
estim_propigation_radius_um_entry.insert(tk.END, str(300))
label_estim_propigation_radius_um = tk.Label(root, text="Enter estim propigation radius in (um)")


extra_brain_region_acryonm = tk.StringVar()
extra_brain_region_acryonm_entry = tk.Entry(root, textvariable=extra_brain_region_acryonm, width=100)
extra_brain_region_acryonm_entry.insert(tk.END, str('VIS, ECT, VISp1, VISp2/3, VISp4, VISp5,  VISp6a, VISp6b'))
label_extra_brain_region_acryonm = tk.Label(root, text="Enter list of brain region acryonms you want to render ")
...
run_button.grid(row=20, column=2)

# grid for the gui
label_cellfinder_output_path.grid(row=1, column=2)
cellfinder_output_path_entry.grid(row=2, column=2)

label_mouse_id.grid(row=3, column=2)
mouse_id_entry.grid(row=4,column=2)
